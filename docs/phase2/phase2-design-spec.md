...
- 配置中新增 `runtime.loadFilesystemSettings` 开关控制是否启用 SDK 的 filesystem settings 加载
- 默认启用
...
- 用户在 `<cwd>/.claude/skills/` 与 `<cwd>/.claude/agents/` 下放置配置时，都会受这个 SDK filesystem settings 开关控制；它不是仅控制 Skills 的独立开关
